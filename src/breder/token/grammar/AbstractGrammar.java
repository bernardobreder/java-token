...
public abstract class AbstractGrammar {
	...
	protected LexerToken read(Enum<?> type, String errorMessage) throws SyntaxException {
		return read(type.ordinal());
	}
	
	protected LexerToken read(int type, String errorMessage) throws SyntaxException {
		if (index >= tokens.length) { throw errorEof(); }
		if (tokens[index].type != type) { throw error(errorMessage); }
		return tokens[index++];
	}
