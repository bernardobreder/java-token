...
public class Grammar {
	...
	protected Token read(String word, String message) throws SyntaxException {
		if (eof()) { throw new SyntaxException(last(), "expected '" + word + "' but was found <EOF> after token"); }
		Token token = token();
		if (token.is(word)) {
			index++;
			return token;
	...
	protected Token readIdentifier(String message) throws SyntaxException {
		if (eof()) { throw new SyntaxException(last(), "expected <id> but was found <EOF> after token"); }
		Token token = token();
	...
	protected Token readNumber(String message) throws SyntaxException {
		if (eof()) { throw new SyntaxException(last(), "expected <number> but was found <EOF> after token"); }
		Token token = token();
		if (token.isNumber()) {
			index++;
			return token;
	...
	protected Token readString(String message) throws SyntaxException {
		if (eof()) { throw new SyntaxException(last(), "expected <string> but was found <EOF> after token"); }
		Token token = token();
		if (token.isString()) {
			index++;
			return token;
	...
	protected ParseException error(String message) {
		return error(message, eof() ? last() : token());
	}

	protected ParseException error(String message, Token token) {
		return new SyntaxException(token, "[" + token.word + ", off=" + token.offset + ", lin=" + token.line + "] " + message);
	}
